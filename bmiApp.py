...
# ส่วนที่ 3 สร้างปุ่มและการคำนวณหาค่าดัชนีมวลกาย
if st.button("คำนวณค่า BMI"):
    height_m = height_cm / 100
    bmi = weight / (height_m ** 2)
    st.write(f"ค่า BMI ของคุณคือ: **{bmi:.2f}**")

    # ส่วนที่ 4 การแปลผลค่า BMI ตามเงื่อนไข
    if bmi < 18.5:
        st.write("⚠️ คุณมีน้ำหนักน้อยกว่าเกณฑ์ (ผอม)")
    elif 18.5 <= bmi < 23.0:
        st.write("🎉 คุณมีน้ำหนักอยู่ในเกณฑ์ปกติ (สุขภาพดี)")
    elif 23.0 <= bmi < 25.0:
        st.write("💡 คุณเริ่มมีน้ำหนักเกินเกณฑ์ (ท้วม)")
    else:
        st.write("🚨 คุณอยู่ในเกณฑ์อ้วน ควรระวังเรื่องสุขภาพและออกกำลังกาย")

# ส่วนที่ 5 เส้นกั้นและแสดงข้อมูลผู้จัดทำ
st.divider()
